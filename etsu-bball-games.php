...
class ETSU_Bball_Games{
	...
	public function __construct($atts = array()){
		...
		$this->year = ( isset($atts['year']) ? $atts['year'] : '2018');
		$this->api_url = 'http://api.espn.com/v1/sports/basketball/mens-college-basketball/teams/2193/events/dates/'.$this->year.'?apikey=';
	...
	private function get_games(){
		...
		if ($games == null) {
			...
			// At the beginning of the season, grab first game
			if ($prev_state == 'pre') {
			...
			if ($current_game == null && $previous_game == null && count($games) > 0) {
				// if no games have completed, grab first game
				// shouldn't ever get in here after adding check on line 100
				if ($prev_state == 'pre') {
					$current_game = $games[0];
